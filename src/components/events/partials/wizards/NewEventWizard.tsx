...
const NewEventWizard = ({ close }: { close: () => void }) => {
  ...
  const ADMIN_INIT_EVENT_ACL_WITH_SERIES_ACL =
    "admin.init.event.acl.with.series.acl";
  if (
    !!orgProperties &&
    !!orgProperties[ADMIN_INIT_EVENT_ACL_WITH_SERIES_ACL]
  ) {
    initEventAclWithSeriesAcl =
      user.org.properties[ADMIN_INIT_EVENT_ACL_WITH_SERIES_ACL] === "true";
  }
  ...
  const nextPage = (values: typeof initialValues) => {
    ...
    if (
      steps[newPage].name === "upload-asset" &&
  ...
  const handleSubmit = (values: typeof initialValues) => {
    const response = dispatch(
      postNewEvent({ values, metadataInfo: metadataFields, extendedMetadata }),
    );
                ...
                {steps[page].name === "summary" && (
                  <NewEventSummary
                    ...
                    metaDataExtendedHidden={
                      !steps.some((step) => step.name === "metadata-extended")
                    }
                    assetUploadHidden={
                      !steps.some((step) => step.name === "upload-asset")
                    }
                  />
                )}
              </div>
...
const getInitialValues = (
  ...
  initialValues = {
    ...initialValues,
    ...getInitialMetadataFieldValues(metadataFields),
  };

  for (const catalog of extendedMetadata) {
    initialValues = {
      ...initialValues,
      ...getInitialMetadataFieldValues(catalog),
    };
  }
  ...
  if (uploadSourceOptions) {
    ...
    uploadSourceOptions = uploadSourceOptions
      .slice()
